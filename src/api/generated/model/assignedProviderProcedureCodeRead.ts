/**
 * Generated by orval v8.14.0 🍺
 * Do not edit manually.
 * DentC Backend
 * Dental PMS REST API. Column-tenant-scoped; snake_case; Orval-ready.
 * OpenAPI spec version: 1.0.0
 */
import type { AssignedProviderProcedureCodeReadValidTeeth } from './assignedProviderProcedureCodeReadValidTeeth';

export interface AssignedProviderProcedureCodeRead {
  code: string;
  legacy_code?: string | null;
  description: string;
  category: string;
  /** @pattern ^(?!^[-+.]*$)[+-]?0*\d*\.?\d*$ */
  default_fee: string;
  default_duration_minutes?: number | null;
  requires_tooth: boolean;
  requires_surface: boolean;
  requires_quadrant: boolean;
  requires_lab: boolean;
  is_ortho: boolean;
  billing_order?: string | null;
  recall_interval?: number | null;
  recall_unit?: string | null;
  is_active: boolean;
  chart_category?: string | null;
  tooth_area?: string | null;
  draw_as?: string | null;
  min_surfaces?: number | null;
  max_surfaces?: number | null;
  default_material_id?: number | null;
  valid_teeth?: AssignedProviderProcedureCodeReadValidTeeth;
  taxable: boolean;
  sales_tax_code?: string | null;
  visit_code?: string | null;
  ledger_code?: string | null;
  ar_code?: string | null;
  is_post_op: boolean;
  exempt_from_dental_max: boolean;
  lock_default_provider: boolean;
  default_provider_id?: string | null;
  default_notes_macro_id?: number | null;
  show_ada_code_in_notes: boolean;
  nhs_treatment_category?: string | null;
  nhs_clinical_data_set?: string | null;
  created_at: string;
}
