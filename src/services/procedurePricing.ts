...
/**
 * Price one procedure the way the on-premise product does when a code is picked —
 * shared by the Transactions Entry, Account Ledger, Restorative Chart and
 * Treatment Plan screens so the same code posts with the same split everywhere.
 * Historically the Restorative Chart priced this way and the other screens used
 * the fee schedule alone (no plan coverage %), so estimates differed by screen.
 *
 * Legacy rule:
 *
