...
export default function Scheduler({
  ...
  const isSlotBlocked = (
    ...
    const operatoryAppointments = appointments.filter(
      appt => 
        appt.operatory === operatoryId &&
        appt.date === date
    );
    
    // Check if any appointment overlaps with this slot
    return operatoryAppointments.some(appt =>
      timeRangesOverlap(slotTime, slotEndTime, appt.startTime, appt.endTime)
    );
  };
  ...
  const getSlotOccupyingAppointment = (
    ...
    return operatoryAppointments.find(appt =>
      timeRangesOverlap(slotTime, slotEndTime, appt.startTime, appt.endTime)
    ) || null;
  };
  ...
  useEffect(() => {
    ...
    const schedulerContainer = document.querySelector('.scheduler-scroll-container');
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (showCalendarPicker && !target.closest('.calendar-picker-container')) {
  ...
  const handleSaveAppointment = (appointmentData: any) => {
    if (editingAppointment) {
      ...
      const updatedAppointment: Appointment = {
        ...editingAppointment,
        patientId: appointmentData.patientId || editingAppointment.patientId,
        patientName: `${appointmentData.lastName}, ${appointmentData.firstName}`,
        date: appointmentData.date || editingAppointment.date,
        startTime: appointmentData.time || editingAppointment.startTime,
        ...
        operatory: appointmentData.operatory || editingAppointment.operatory,
        provider: appointmentData.provider || editingAppointment.provider,
      ...
      const newAppointment: Appointment = {
        ...
        endTime: calculateEndTime(selectedSlot?.time || appointmentData.time, appointmentData.duration),
        duration: appointmentData.duration,
        procedureType: appointmentData.procedureType,
        status: "Scheduled",
        operatory:
      ...
      <div className="bg-white shadow-md border-b border-[#E2E8F0] sticky top-0 z-10">
        ...
        <div className="bg-gradient-to-r from-[#1F3A5F] to-[#2d5080] px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* Date Picker Button */}
            <div className="relative calendar-picker-container" style={{ zIndex: 9999 }}>
              <button
              ...
              {showCalendarPicker && (
                <div className="absolute top-full left-0 mt-2" style={{ zIndex: 9999 }}>
                  <input
                    type="date"
                    className="border-2 border-[#3A6EA5] rounded-lg p-2 shadow-xl bg-white"
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className="px-3 py-1.5 bg-[#F59E0B] hover:bg-[#D97706] text-white rounded-md transition-colors flex items-center gap-1.5 text-xs font-semibold shadow-sm"
            >
              <Search className="w-4 h-4" strokeWidth={2.5} />
              QUICK FILL
            </button>
            <button
              className="px-3 py-1.5 bg-[#64748B] hover:bg-[#475569] text-white rounded-md transition-colors flex items-center gap-1.5 text-xs font-semibold shadow-sm"
            >
              ...
              >
                ...
                <div className="relative bg-white">
                  {timeSlots.map((time) => {
                    ...
                    const slotBlocked = isSlotBlocked(time, operatory.id, currentDate);
                    const occupyingAppt = getSlotOccupyingAppointment(time, operatory.id, currentDate);
                    ...
                    return (
                      <div
                        ...
                        onContextMenu={(e) => {
                          if (!slotBlocked) {
                            handleEmptySlotRightClick(e, time, operatory.id);
                          } else {
                            e.preventDefault();
                          }
                        }}
                  ...
                  {appointments
                    .filter(
                      (appt) => 
                        ...
                        appt.date === formatDateYYYYMMDD(selectedDate)
                    )
                    .map((appointment) => {
                      const { top, height } =
