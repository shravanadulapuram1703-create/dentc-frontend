...
| **SCHED-NP-1** | `AppointmentSchedulerRead` (the `/appointments/scheduler` calendar feed) **does not expose `is_new_patient`**, although `AppointmentRead`, `AppointmentCreate` and `AppointmentUpdate` all carry it. The day grid is rendered from the feed, so no loaded appointment could ever show the legacy new-patient ("NP") marker — only ones created in the same session via Quick Save. Verified live 2026-09-04: `PATCH /appointments/{id} {is_new_patient:true}` → 200 with `is_new_patient: true`, yet the same id in `/appointments/scheduler` has no such key. | Staff cannot tell new patients from existing ones on the scheduler. Workaround costs one extra `GET /appointments` page-walk per calendar fetch (`fetchNewPatientAppointmentIds`) to overlay the flag by id. | Add `is_new_patient` to `AppointmentSchedulerRead` (same source column). The client overlay can then be removed. |
